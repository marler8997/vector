...
pub struct PipeConfig {
    /// The file descriptor number of the pipe.
    pub fd: c_int,
...
impl_generate_config_from_default!(PipeConfig);

#[async_trait::async_trait]
#[typetag::serde(name = "pipe")]
impl SourceConfig for PipeConfig {
    async fn build(&self, cx: SourceContext, fd: c_int) -> crate::Result<super::Source> {
        pipe_source(
            fd,
            io::BufReader::new(io::pipe(fd)),
            self.clone(),
            cx.shutdown,
            cx.out,
        )
...
mod tests {
    ...
    async fn pipe_decodes_line() {
        assert_source_compliance(&["protocol"], async {
            ...
            pipe_source(buf, config, ShutdownSignal::noop(), tx)
                .unwrap()
                .await
                .unwrap();
